...
def create_app(config_name=None):
    """Enhanced application factory with analytics support"""
    ...
    # Register routes (includes analytics)
    register_routes(app)
    ...
    app.logger.info(f'TypeTutor backend with analytics initialized in {config_name} mode on port {app.config.get("PORT", 5001)}')
    return app
...
if __name__ == '__main__':
    ...
    print(f"Starting TypeTutor backend with analytics on {host}:{port}")
